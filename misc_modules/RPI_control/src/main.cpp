...
// Auto-scan range (common PCF8574 addresses)
constexpr int I2C_MIN = 0x20;
constexpr int I2C_MAX = 0x27;
...
SDRPP_MOD_INFO{
    /* Name:            */ "RPI_control",
    /* Description:     */ "Allow user to control radio via encoder and get feedback on an LCD",
    /* Author:          */ "Quaid Thompson",
    /* Version:         */ 0, 0, 1,
    /* Max instances    */ -1
};
 ...
 */
 ...
 size_t curl_write_cb(void* contents, size_t size, size_t nmemb, std::string* out) {
    out->append((char*)contents, size * nmemb);
    return size * nmemb;
}

bool i2c_write_byte(int data) {
    if (write(i2c_fd, &data, 1) != 1) return false;
    return true;
}
...
void lcd_set_cursor(int col, int row) {
    ...
    lcd_send_byte(0x80 + row_offsets[row] + col, LCD_COMMAND);
}
...
int find_i2c_lcd_addr() {
    char filename[20];
    snprintf(filename, 19, "/dev/i2c-%d", I2C_BUS);
    i2c_fd = open(filename, O_RDWR);
    if (i2c_fd < 0) return -1;

    for (int addr = I2C_MIN; addr <= I2C_MAX; ++addr) {
        if (ioctl(i2c_fd, I2C_SLAVE, addr) < 0) continue;
        // Try sending a command to initialize (0x00 is a no-op for most)
        if (i2c_write_byte(0x00)) return addr;
    }
    return -1;
}
...
private:
    ...
    void commandHandler(std::string cmd) {
        ...
        if (parts[0] == "F" || parts[0] == "\\set_freq") {   // user wants to set the frequency
            std::lock_guard lck(vfoMtx);

            // if number of arguments isn't correct, return error
            if (parts.size() != 2) {
                resp = "ERROR: What frequency?  For 122.7 MHz,\nF 1227000\nE:0\n";
        ...
        else if (parts[0] == "f" || parts[0] == "\\get_freq") {   // user wants to get the frequency
            std::lock_guard lck(vfoMtx);
            ...
            if (parts.size() >= 2 && parts[1] == "?") {       // i guess M ? should be a "help" command?
            ...
            // if number of arguments isn't correct, return error
            if (parts.size() != 3) {
                resp = "ERROR: Something's missing in your command!    note to dev: Put propper syntax of command here\nE:2\n";
                client->write(resp.size(), (uint8_t*)resp.c_str());
                return;
            }
            ...
            for (char c : parts[2]) {
                if (!std::isdigit(c) && !(c == '-' && !pos)) {
                    resp = "ERROR: Bandwidth must be an integer!\nE:3\n";
                    client->write(resp.size(), (uint8_t*)resp.c_str());
                    return;
                }
                pos++;
            ...
            if (it == radioModeToString.end()) {
                resp = "ERROR: Invalid Mode! FM WFM AM DSB USB CW LSB RAW\nE:4\n";
                client->write(resp.size(), (uint8_t*)resp.c_str());
                return;
            }
            ...
            if (!selectedVfo.empty() && core::modComManager.getModuleName(selectedVfo) == "radio" && tuningEnabled) {
                ...
                resp = "ERROR: Something has gone wrong, but VFO is enabled!\nE:5\n";
            }

            client->write(resp.size(), (uint8_t*)resp.c_str());
        }
        else if (parts[0] == "m" || parts[0] == "\\get_mode") {    // user wants to get the mode
            std::lock_guard lck(vfoMtx);
            resp = "RAW\n";
            ...
            if (parts.size() != 2) {
                resp = "ERROR: Incorrect number of arguements! This command dosent do anything anyway.\nE:6\n";
                client->write(resp.size(), (uint8_t*)resp.c_str());
                return;
            }

            if (parts[1] == "?") {
                resp = "'V' dosent actually do anything, theres only one VFO.\nDone:0\n";     // TODO: Print syntax out to console
            }
            else if (parts[1] != "VFO") {
                resp = "ERROR: Not a valid VFO. SDR++ dosent have multiple VFOs anyway.\nE:7\n";
            }
